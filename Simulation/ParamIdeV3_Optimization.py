...
# --- Data Loading from subsampled_static_points.csv ---
def load_experiment_data(static_points_file='subsampled_static_points.csv'):
    """
    Load static points data from subsampled_static_points.csv file.
    
    Args:
        static_points_file: Path to the CSV file containing static points data
        
    Returns: static_positions (N,2), static_pressures (N,3), kind_labels (N,)
    """
    try:
        ...
        # Extract kind labels
        kind_labels = static_data['kind'].values
        
        print(f"Processed {len(static_positions)} static positions and pressures")
        print(f"Kind distribution: {np.bincount(kind_labels)}")
        print("\nStatic positions (dz, dx):")
        for i, pos in enumerate(static_positions):
            print(f"Point {i+1} (kind={kind_labels[i]}): [{pos[0]:.6f}, {pos[1]:.6f}]")
        
        print("\nStatic pressures (P1, P2, P3):")
        for i, press in enumerate(static_pressures):
            print(f"Point {i+1} (kind={kind_labels[i]}): [{press[0]:.6f}, {press[1]:.6f}, {press[2]:.6f}]")
        
        return static_positions, static_pressures, kind_labels
        
    except FileNotFoundError:
        print(f"Error: Static points file '{static_points_file}' not found.")
        print("Please run Raw_data_subsampling.py first to generate the subsampled static points file.")
        raise
...
# --- Kind-based Point Selection ---
# kind = 1: grid27, kind = 2: grid8
selected_kind = 1  # Options: 1 (grid27), 2 (grid8)
...
param_bounds = {
    ...
    'uy': (-3.0, 3.0),                                                    # Initial: -1.1038382647760769
...
def plot_comparison_jax(sim_positions, exp_positions, selected_indices=None, title_suffix=""):
    ...
    kind_name = "grid27" if selected_kind == 1 else "grid8"
    title = f'JAX Simulation vs Experimental Static Tip Positions\nKind: {selected_kind} ({kind_name}, {len(selected_indices)} points)'
    if title_suffix:
...
if __name__ == "__main__":
    ...
    # Load static points data from subsampled_static_points.csv
    static_points_file = 'subsampled_static_points.csv'
    print(f"\nLoading static points from: {static_points_file}")
    
    static_positions, static_pressures, kind_labels = load_experiment_data(static_points_file)
    
    # Get selected static point indices based on kind (0-based indexing)
    selected_indices = np.where(kind_labels == selected_kind)[0]
    kind_name = "grid27" if selected_kind == 1 else "grid8"
    print(f"\nUsing static point kind: {selected_kind} ({kind_name})")
    print(f"Selected points: {selected_indices + 1} (1-based)")
